...
class REGRESSION_SENSORS():
    ...
    def PROCESSING(self, out_unit, X1, target, TdisValue, TsucValue, compValue, freqValue, Method):
        # 예측 대상
        self.target = target
        self.Method = Method
        ...
        self._OutUnitData['comp_fequency_sum'] = self._OutUnitData[self.Compfreq[0]] + self._OutUnitData[self.Compfreq[1]]
        # print("Target : {}".format(self.target))
        # print("Suction temp : {}".format(self.SuctionTemp))
        # print("Discharge temp : {}".format(self.DischargeTemp))
        # print("Compressor Signal : {}".format(self.CompressorSignal))
        # print("Compressor Compfreq : {}".format(self.Compfreq))
        ...
        if self.comp_num == 1:
            # Mode1 : comp1 사용
            self.OutIntegData = self._OutUnitData[(self._OutUnitData[self.Compfreq[0]] > 5)
                                                  & (self._OutUnitData[self.CompressorSignal[0]] != 0)] #작동중인 데이터만 사용
            # self.OutIntegData = self._OutUnitData.dropna(axis=0) # 결측값을 그냥 날리는 경우

        elif self.comp_num == 2:
            # Mode2 : comp2 사용
            self.OutIntegData = self._OutUnitData[self._OutUnitData[self.Compfreq[1]] > 5
                                                  & (self._OutUnitData[self.CompressorSignal[1]] != 0)] #작동중인 데이터만 사용
            # self.OutIntegData = self._OutUnitData.dropna(axis=0) # 결측값을 그냥 날리는 경우

        elif self.comp_num == 3:
            # Mode3 : comp1 + comp2
            self.OutIntegData = self._OutUnitData[(self._OutUnitData['comp_fequency_sum'] > 5)
                                                  & (self._OutUnitData[self.CompressorSignal[0]] != 0)
                                                  & (self._OutUnitData[self.CompressorSignal[1]] != 0)] #작동중인 데이터만 사용
            # self.OutIntegData = self._OutUnitData.dropna(axis=0) # 결측값을 그냥 날리는 경우
        self.OutIntegData.to_csv("{}/After_Outdoor_{}_{}.csv".format(save, out_unit, self.comp_num))  # 조건 적용 후
    ...
    def PlottingSystem(self, save, out_unit):
        ...
        if self.comp_num == 1:
            ...
            ax1.scatter(solve['comp_fequency_sum'], solve[self.target], s=80, alpha=0.5, color='royalblue')
