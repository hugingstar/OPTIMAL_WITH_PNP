...
class DataCorrection:
    ...
    def Visualizing(self, out_unit):
        ...
        # Indoor velocity
        self.IndoorVelocity = self._outFlowData[[self.col1[3], self.col2[3]]]
        self.IndoorVelocity = self.IndoorVelocity.rename(columns={self.col1[3]: self.TIME, self.col2[3]: 'indoor_velocity'})
        for _o in range(4, 6): # 뒤쪽 3개
            tem = self._outFlowData[[self.col1[_o], self.col2[_o]]]
    ...
    def PlottingIndoorMesurement(self, plt_ST, plt_ET, save, out_unit):
        ...
        cd_col_list = []
        for _ in [1, 2, 3]: #outlet 1~3
            cd_col_list.append('condout{}'.format(_))
            #가장 큰것 :
            ax4.plot(tt, solve['condout{}'.format(_)].tolist(), 'b', alpha=0.2, linewidth='3', drawstyle='steps-post')
        avg_temp_list = solve[cd_col_list].mean(axis=1)
        ax4.plot(tt, avg_temp_list, 'b', linewidth='4', drawstyle='steps-post')

        df1 = pd.DataFrame({'IndoorHX_outlet_Average_Temperature': avg_temp_list})
        df1.to_csv("{}/Outdoor_{}_IndoorHX_outlet_AvgTemp.csv".format(save, out_unit))
        print(df1)

        cd_col_list = []
        for _ in [1, 2, 4, 5, 6, 7, 8, 9]: #inlet 1~9
        ...
        df2 = pd.DataFrame({'IndoorHX_inlet_Average_Temperature': avg_temp_list})
        df2.to_csv("{}/Outdoor_{}_IndoorHX_inlet_AvgTemp.csv".format(save, out_unit))
        print(df2)
        ...
        ax4.set_yticks([0, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50])
    ...
    def PlottingIndoorSystem(self, plt_ST, plt_ET, save, out_unit, ind_unit):
        ...
        ax2.legend(['Inlet temperature', 'Outlet temperature', 'Zone temperature', 'Set temperature'], fontsize=18,
    ...
    def PlottingOutdoorMesurement(self, plt_ST, plt_ET, save, out_unit):
        ...
        cd_col_list = []
        for _ in [2, 3, 4, 5, 7, 8, 9]: #outlet 1~10
            cd_col_list.append('condout{}'.format(_))
            #가장 큰것 :
        ...
        df1 = pd.DataFrame({'OutdoorHX_outlet_Average_Temperature': avg_temp_list})
        df1.to_csv("{}/Outdoor_{}_OutdoorHX_outlet_AvgTemp.csv".format(save, out_unit))
        print(df1)
        ...
        df2 = pd.DataFrame({'OutdoorHX_inlet_Average_Temperature': avg_temp_list})
        df2.to_csv("{}/Outdoor_{}_OutdoorHX_inlet_AvgTemp.csv".format(save, out_unit))
        print(df2)
        ...
        ax6.set_yticks([0, 500, 1000, 2000, 3000, 4000, 5000])
        ax7.set_yticks([0, 100, 200, 300, 400, 500])
